...
export class SmartHQClothesDryer extends deviceBase {
  ...
  private async initializeMatter(): Promise<void> {
    ...
    if (!matterAPI.deviceTypes.LaundryDryerDevice) {
      ...
      // LaundryDryerDevice device type at all, so this is not a "Matter is
      ...
      if (this.device.matterOnly) {
        this.warnLog('Ignoring matterOnly: Homebridge has no LaundryDryerDevice device type, so Matter can never be used for this appliance. Publishing over HAP instead.')
  ...
  private initializeHAP(): void {
    ...
    const existingRunningSwitch = this.accessory!.getService('Dryer Running')
    if (this.device.showRunningSwitch) {
      const runningSwitch = existingRunningSwitch ?? this.accessory!.addService(this.platform.Service.Switch, 'Dryer Running', 'DryerRunning')
      this.setServiceName(runningSwitch, 'Dryer Running')
      runningSwitch
    ...
    this.setServiceName(dryerValve, 'Dryer')
    ...
    this.setServiceName(doorLock, 'Dryer Door Lock')
    doorLock
    ...
    this.setServiceName(doorSensor, 'Dryer Door')
    doorSensor
