...
- Build: `npm run build` (`rimraf ./dist` → `tsc` → `tsc-alias` → copy plugin UI html). All steps are required for a working package.
...
- One device class per appliance type in `src/devices/`, extending `deviceBase` in `src/devices/device.ts`; instances are stored as `accessory.control`. `src/devices/OpalIceMaker/` is a composed device using a manager pattern (see its README); refrigerators with a built-in Keurig get a separate accessory from `src/devices/keurig.ts`.
